...
class DbClassCreditCard implements DbClass {
    ...
    void buildReadDefaultPaymentQuery() {
        query = "SELECT "+EXP_DATE+", "+CARD_TYPE+", "+CARD_NUM+" FROM altpayment WHERE custid = " + custProfile.getCustId();
    }
    ...
    void populatePaymentList(ResultSet rs) throws DatabaseException {
        ...
        if(rs != null){
            try {
                while(rs.next()) {
                    payment = new CreditCardImpl(this.custProfile.getFirstName(),rs.getString(EXP_DATE),rs.getString(CARD_NUM),rs.getString(CARD_TYPE));
    ...
    void populateDefaultPayment(ResultSet rs) throws DatabaseException {
        try {
            if(rs.next()){
                defaultPaymentInfo = new CreditCardImpl(this.custProfile.getFirstName(),rs.getString(EXP_DATE),rs.getString(CARD_NUM),rs.getString(CARD_TYPE));
            }          
        }
        catch(SQLException e) {
            throw new DatabaseException(e);
